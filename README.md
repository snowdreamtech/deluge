...
docker pull snowdreamtech/deluge:debian
docker run -d --name=base -e TZ=Asia/Shanghai snowdreamtech/deluge:debian
...
docker build -t snowdreamtech/deluge:debian ./docker/debian/
...
docker build \
  ...
  -t snowdreamtech/deluge:debian-custom \
  ./docker/debian/
```
...
services:
  base:
    image: snowdreamtech/deluge:debian
    container_name: base
    environment:
      - TZ=Asia/Shanghai
    restart: unless-stopped
...
```dockerfile
FROM snowdreamtech/deluge:debian
...
```bash
docker run -e DEBUG=true snowdreamtech/deluge:debian
```
